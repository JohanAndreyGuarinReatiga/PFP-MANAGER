...
export class ServicioPropuesta {
  ...
  static async crearPropuesta(data) {
    try {
      ...
      // CORRECCIÓN: Devolver el objeto con el ID insertado
      return {
  ...
  static async listarPropuestas(
    estado = "todos",
    { pagina = 1, limite = 10, ordenarPor = "fechaCreacion", orden = -1 } = {},
  ) {
    const db = await connection()
    const filtro = {}
    ...
    if (clienteIds.length > 0) {
      const clientes = await db
        .collection("clientes")
        .find({ _id: { $in: clienteIds.map((id) => new ObjectId(id)) } })
        .toArray()
      clientes.forEach((c) => {
